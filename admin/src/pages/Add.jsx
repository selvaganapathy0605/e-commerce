...
const Add = ({ token }) => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      {loading && <div className="loading-spinner w-20 bg-black mx-96 rounded text-center py-2 text-gray-400 ">Loading...</div>}
      <div>
      ...
      <div>
        ...
        <div className='flex gap-3'>
          <div onClick={() => setSizes(prev => prev.includes("S") ? prev.filter(i => i !== "S") : [...prev, 'S'])}>
            <p className={`${sizes.includes('S') ? "bg-pink-100" : "bg-slate-200"} px-3 py-3 cursor-pointer`}>S</p>
          </div>
          <div onClick={() => setSizes(prev => prev.includes("M") ? prev.filter(i => i !== "M") : [...prev, 'M'])}>
            <p className={`${sizes.includes('M') ? "bg-pink-100" : "bg-slate-200"} px-3 py-3 cursor-pointer`}>M</p>
          </div>
          <div onClick={() => setSizes(prev => prev.includes("L") ? prev.filter(i => i !== "L") : [...prev, 'L'])}>
            <p className={`${sizes.includes('L') ? "bg-pink-100" : "bg-slate-200"} px-3 py-3 cursor-pointer`}>L</p>
          </div>
          <div onClick={() => setSizes(prev => prev.includes("XL") ? prev.filter(i => i !== "XL") : [...prev, 'XL'])}>
            <p className={`${sizes.includes('XL') ? "bg-pink-100" : "bg-slate-200"} px-3 py-3 cursor-pointer`}>XL</p>
          </div>
          <div onClick={() => setSizes(prev => prev.includes("XXL") ? prev.filter(i => i !== "XXL") : [...prev, 'XXL'])}>
            <p className={`${sizes.includes('XXL') ? "bg-pink-100" : "bg-slate-200"} px-3 py-3 cursor-pointer`}>XXL</p>
          </div>
        </div>
      </div>
      ...
      <button className='w-28 py-3 mt-4 bg-black text-white hover:hover:bg-blue-950' type='submit'>Add Product</button>
    </form>
  )
}
